@php
    ...
    // Badge stil (kao Machines) – inline, bez tailwinda
    $styleMap = [
    ...
    // Heroicon-like SVG (stroke = currentColor) – kao Machines
    $svg = [
    ...
    @forelse($items as $item)
        @php
            ...
            if (! $end) {
                ...
                $status  = ExpiryBadge::color($end, $soonDays);   // danger|warning|success
                $text    = $end->format('d.m.Y.');                // ✅ bez "(za X dana)"
                $tip     = ExpiryBadge::tooltip($end, $soonDays);
                $iconKey = ExpiryBadge::icon($end, $soonDays);    // heroicon-o-...
            }
